...
int main(int argc, char *argv[])
{
	...
	if(cmdline.reconFlag)
	{
		...
		ReadPriorParams(cmdline.ReconParamsFile, &reconparams); 
		NormalizePriorWeights3D(&reconparams);

		/* Print MBIR parameters followed by prior parameters */
	...
	max_num_pointer = (float *) get_spc(Nxy,sizeof(float)); /* Used for a 8-bit quantization of the A-matrix entries */
	...
	if(cmdline.reconFlag || cmdline.writeProjectionFlag)
	{
		...
		{
			if(cmdline.readInitImageFlag)  /* If input image is specified, compute indvidual slice-wise forward projection */
			{
				for(jz=0;jz<Nz;jz++)
				{
	...
	if(cmdline.reconFlag)
	{
		...
		if(strcmp(reconparams.MBIRMode,"PnP")==0)
		{
			...
			for(jz=0;jz<Nz;jz++)
				memcpy(ProxMap.image[jz],Image.image[jz],Nxy*sizeof(float));	
			
			reconparams.proximalmap = ProxMap.image;  // **ptr to proximal map image

			strcpy(reconparams.priorparams.DataDir,cmdline.ReconImageFile); // path for writing temporary image data files (in/out for PnP denoiser)

			if(strcmp(reconparams.PriorModel, "CNN")==0)
			{
				reconparams.priorparams.TF_gpu_flag = cmdline.TF_gpu_flag;
				strcpy(reconparams.priorparams.TF_CheckPointDir, cmdline.TF_CheckPointDir);
				strcpy(reconparams.priorparams.TF_CheckpointState, cmdline.TF_CheckpointState);
				reconparams.priorparams.TF_CheckpointEpochNum = cmdline.TF_CheckpointEpochNum;
			}
		}
...
void readCmdLine(int argc, char *argv[], struct CmdLine *cmdline)
{
    ...
    cmdline->reconFlag=0; //No reconstruction
    cmdline->readInitImageFlag=0;
    cmdline->readInitProjectionFlag=0;
    cmdline->writeProjectionFlag=0;

    cmdline->TF_gpu_flag=1;              
    strcpy(cmdline->TF_CheckPointDir, "../data/checkpoint");   
    strcpy(cmdline->TF_CheckpointState, "latest"); 
    cmdline->TF_CheckpointEpochNum=0;   
    ...
    /* get options */
    while ((ch = getopt(argc, argv, "i:j:k:s:w:r:m:t:e:f:c:gdlv")) != EOF)
    {
        switch (ch)
        {
            case 'i':
            ...
            case 'c':
            {
                sprintf(cmdline->TF_CheckPointDir, "%s", optarg);
                break;
            }
            case 'g':
            {
                cmdline->TF_gpu_flag=0; /* default is 1 enabling GPU acceleration */
                break;
            }
            case 'd':
            {
                strcpy(cmdline->TF_CheckpointState, "specific"); /* default is "latest" */
                break;
            }
            case 'l':
            {
            	cmdline->TF_CheckpointEpochNum=atoi(optarg); 	/* indiciates epcoh number for loading checkpoint. this argument is needed when TF_CheckpointState="specifc".*/
            	break;
            }
    ...
    if(cmdline->reconFlag)
    {
        ...
        if( (strcmp(cmdline->TF_CheckpointState, "specific")==0) && (cmdline->TF_CheckpointEpochNum<1) )
        {
        	fprintf(stderr,"Error: To specify a particular checkpoint, a positive epoch number is needed\n");
        	fprintf(stderr,"Try '%s -help' for more information.\n",argv[0]);
            exit(-1);
        }
...
//  fprintf(stdout,"***80 columns*******************************************************************\n\n");
    ...
    fprintf(stdout,"\t-g <TensorFlow flag>   	   : <optional> if -g included, disables GPU acceleration for CNN \n");
    fprintf(stdout,"\t-c <TensorFlow Directory>    : Checkpoint directory for loading CNN parameters \n");
    fprintf(stdout,"\t-d <TensorFlow Checkpt State>: <optional> 'latest' (default value) or 'specific' - from checkpoint directory load latest checkpoint or specific one \n");
    fprintf(stdout,"\t-l <TensorFlow Epoch number> : <optional>  positive integer - if Checkpt State is 'specific', then this epoch value specifies that checkpoint \n");
